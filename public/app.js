  ...
  $.getJSON("/all", function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      $("#results").prepend("<p class='dataentry' data-id=" + data[i]._id + 
        "><span class='dataTitle' data-id=" +data[i]._id + ">TICKER: " 
        + data[i].ticker.toUpperCase() +"| Ceiling: "+data[i].ceiling+
        "| Floor: "+data[i].floor+"</span><span class=deleter>X</span></p>");
    }
  });
}
...
$(document).on("click", "#makenew", function() {
  ...
  .done(function(data) {
    ...
    $("#results").prepend("<p class='dataentry' data-id=" + data._id + "><span class='dataTitle' data-id=" +
      data._id + ">" + data.ticker + "</span><span class=deleter>X</span></p>");
